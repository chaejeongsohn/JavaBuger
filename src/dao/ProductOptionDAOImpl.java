...
public class ProductOptionDAOImpl {
    ...
    public int insertProductOption(ProductOption productOption) throws SQLException {
        ...
        try {
            ...
            if(result==0) {
                con.rollback();
                throw new SQLException("에러: 상품옵션이 입력되지 않았습니다. / public int insertProductOption(ProductOption productOption)");
            }
    ...
    /*
    파라미터 로 받는 productOption 의 optionNumber, categoryNumber, optionName, optionPrice 를 추출하여
    DB 에 저장된 productionOption 중 같은 optionNumber 가진 튜플을 찾아 업데이트 해준다.
     */
    //    @Override
    public int updateProductOption(ProductOption productOption) throws SQLException {
        ...
        try {
            ...
            if(result==0) {
                con.rollback();
                throw new SQLException("에러: 상품옵션이 업데이트 되지 않았습니다. / public int updateProductOption(ProductOption productOption)");
            }
        }finally {
            con.commit();
            DbUtils.close(con, ps , null);
    ...
    public int deleteProductOption(int productOptionNumber) throws SQLException {
        ...
        try {
            ...
            if(result==0) {
                con.rollback();
                throw new SQLException("에러: 상품옵션이 삭제 되지 않았습니다. / public int deleteProductOption(ProductOption productOption)");
            }
        }finally {
            con.commit();
            DbUtils.close(con, ps , null);
